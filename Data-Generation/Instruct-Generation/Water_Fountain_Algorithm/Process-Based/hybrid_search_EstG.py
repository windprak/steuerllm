...
def generate_new_query(query: str):
        ...
        messages = [
            {"role": "system", "content": "Du erhältst immer einen Kontext in Form einer alten Suchmaschinen-Query, die zu keinem Ergebnis geführt hat. Deine Aufgabe ist es, basierend auf den wichtigen Aspekten der alten Query eine neue Such-Query zu erstellen, die sich auf steuerrechtliche Fragestellungen in der deutschen Einkommensteuer konzentriert und für die Suchmaschine besser verständlich ist. Extrahiere die relevanten Informationen aus der ursprünglichen Query und optimiere die neue Query so, dass sie steuerrechtliche Aspekte klar hervorhebt. Gib ausschließlich die neue Query zurück, ohne zusätzliche Kommentare, Erläuterungen oder Formatierungen. Beispiel: Alter Kontext: 'Die Bäcker- & Konditormeister Müller und Weber interessieren sich für einen Volkswagen Transporter, den sie als Auslieferungswagen anschaffen möchten. Da sie den Wagen nicht bar bezahlen können, schlägt Ihnen der Volkswagen-Händler eine entsprechende Kredit- oder Leasingfinanzierung vor. Erläutern Sie bitte die wesentlichen Bedingungen eines Leasingvertrages.' Neue Query: 'steuerrechtliche Behandlung Leasingvertrag Fahrzeuge'"},
            {"role": "user", "content": f"Kontext:\n{query}\n Frage: Erstelle mir dazu eine neue Query für Search Enginges und fokussiere dich dabei darauf, dass es um das Steuerrecht (Einkommensteuer) geht. Gebe mir nur die neue query aus und sonst gar nichts."}
        ]
...
def get_quality_criteria(fragetyp: str):
    if fragetyp == "Verständnis- bzw. Kommentarfragen":
        return (
            ...
            "Wenn du nach der Einkommensart gefragt wirst, benenne diese klar."
        ...
        return (
            "In der Regel ist die Lösung inkorrekt"
...
def generate_answer_with_fau(query: str, context_texts: List[Dict], fragetyp: str):
    ...
    try:
        ...
        temperature, top_p = rand_temp_tp()
        # Make API call with retry logic
        response_data = make_fau_request(url, headers, {
            "messages": messages,
...
def rand_temp_tp():
    """
    First output is Temperature, second is Top-P
    """
...
def generate_more_questions(context: List[Dict], question: str, max_questions: int = 3) -> List[dict]:
    ...
    system_prompt = """
        Du bist ein spezialisiertes KI-System zur Generierung von steuerrechtlichen Fragen mit Fokus auf die Einkommensteuer. Deine Aufgabe ist es, detaillierte, praxisorientierte und fachlich fundierte Fragen zu erstellen, die verschiedene Schwierigkeitsstufen (1 bis 4) abdecken. Die Fragen sollen auf realistischen Szenarien basieren, die für Übungs- oder Prüfungszwecke geeignet sind.

        Anforderungen an die Fragen:
        Fachliche Präzision: Fragen müssen relevante Paragraphen und steuerrechtliche Begriffe korrekt verwenden.
        Praxisbezug: Jede Frage soll ein realistisches Szenario oder Problem behandeln, das im steuerlichen Alltag auftreten könnte.
        Schwierigkeitsgrad: Die Fragen sollen die Schwierigkeitsstufen 1 bis 4 abdecken, wobei Stufe 1 einfache Verständnisfragen darstellt und Stufe 4 komplexe und kombinierte Problemstellungen umfasst.
        Fragetypen: Wähle für jede Frage aus einem der definierten Fragetypen aus und kennzeichne diesen explizit in der Ausgabe.
        Format: Die Ausgabe erfolgt im JSON-Format mit den Feldern:
        frage: Die gestellte Frage.
        typ: Der Fragetyp (z. B. "Verständnisfrage", "Rahmenfrage", "Korrekturfrage").
        quelle: Ein Hinweis, ob Paragraphen oder Gesetze genannt werden sollen.
        Fragetyp und Qualitätskriterien:

        **Verständnis- bzw. Kommentarfragen**
        Ziel: Frage explizit nach der Einkommensart des Szenarios und prüfe das Verständnis grundlegender Konzepte.
        Beispiel:
        - "Mein Haus liegt im Betriebsvermögen, und ich vermiete es an eine Familie. Welche Einkünfte erziele ich, wenn ich das Haus nach 15 Jahren verkaufe? Nenne nur die Einkunftsart!"
        - "Ein Steuerpflichtiger erzielt Einkünfte aus Vermietung und Verpachtung. Zu welcher Einkunftsart gehören diese Einnahmen?"
        Qualitätskriterien:
            - Die Frage sollte ein grundlegendes Konzept der Steuererhebung oder der Einkunftsarten thematisieren.
            - Die Antwort sollte klar und direkt sein, ohne unnötige Komplikationen.
            - Schwierigkeitsgrad 1 bis 2 (für Einsteiger bis mittlere Kenntnisse).
    ...
    Generate up to {max_questions} questions about the Einkommensteuerrecht and the Einkommensarten, the amount of questions depends on the context and if there is a possibility to create a question. The questions should be detailed and (only) if needed contain a creative case scenario. The questions should be high-quality german Einkommenssteuer-related questions based on the context. Check for each question type, if there is a possibility to generate a question, if you are unsure skip the question type as you explicitly do not need to create exactly {max_questions} questions. 
    ...
    IMPORTANT: Do not use the same or slighty different question multiple times, also watch out that you do not create the same question multiple times which simply uses different terms or numbers.
    Do not start every question with "Ein Unternehmen ..." be creative with the beginning of the question!
    If possible, all questions should focus on a different topic of the Einkommensteuer in Germany and should not address the same issue, if not possible then return less then {max_questions} questions.
