...
class Processor:
    ...
    def process(self, log_path="processed_articles_Comm.log"):
        ...
        with Pool(processes=self.num_jobs) as pool:
            for result in pool.imap_unordered(self.call_endpoint, articles_per_endpoint):
                ...
                print("Current Question successfully processed")
    ...
    def get_searxng_results(self, query: str, top_k: int = 3):
        try:
            ...
            for result in results.get("results", [])[:top_k]:
                ...
                scraped_content = self.scrape_and_process_url(url) #TODO Hier passiert noch ein Fehler, evtl solved
    ...
    def call_endpoint(self, article_data: Dict) -> Dict:
        ...
        for query in ["Anwendung", "Interpretation", "Kommentar"]:
        ...
        try:
            payload = self.prepare_payload_q(final_context)
            
            
            question = requests.post(endpoint, json=payload, timeout=600)
            
            question.raise_for_status()

                    # Antwort für die Frage verarbeiten
            question_content = question.json()["choices"][0]["message"]["content"]
            cleaned_question = question_content.strip()

            
            payload_a = self.prepare_payload_a(final_context, cleaned_question)
            # Anfrage für die Antwort
            ...
            return self.call_endpoint_rec(article, payload, payload_a= payload_a, tried_endpoints=[endpoint], snd_context=snd_context)

    def call_endpoint_rec(self, article, payload_q, payload_a, tried_endpoints: List[str], snd_context) -> Dict:
        ...
        try:
            ...
            # Process answer for the question
            ...
            return self.call_endpoint_rec(article, payload_q, payload_a, tried_endpoints, snd_context)

    def prepare_payload_q(self, article: str) -> Dict:
        """Prepare the request payload for the FAU endpoint."""
        system_prompt = """
        Du bist ein Experte für deutsches Steuerrecht. Deine Aufgabe ist es, hochwertige und sinnvolle juristische Fragen zu einem bestimmten Artikel und dessen Kontext zu erstellen. Konkret sollst du eine Frage generieren, bei dem die Frage darauf abzielt, den Artikel inhaltlich zu verstehen.

        Anforderungen:
        Erstelle eine Frage basierend auf dem Artikel.
        Antworte nur mit der Frage und mit sonst nichts anderem!
        Schreibe die Frage ausschließlich auf Deutsch.
        Formuliere die Frage kreativ, sodass sie den Kontext des Artikels beleuchtet und vermeide redundante Fragen.
        """
        ...
        Aufgabe:
        Erstelle eine detaillierte und präzise Frage, die sich auf den oben angegebenen Paragraphen bezieht und dazu anregt, einen Kommentar zu dem Paragraphen zu verfassen. Der Kommentar soll die Bedeutung des Paragraphen erklären und aufzeigen, wie dieser in der Praxis angewendet wird. Achte darauf, dass die Frage Raum lässt, um Beispiele und gegebenenfalls relevante rechtliche Kommentare zu integrieren.

        Die Frage sollte:
        1. Den Paragraphen in seinem Kontext erläutern und dazu auffordern, die rechtlichen Implikationen zu diskutieren.
        2. Den Beantworter dazu anregen, mit Beispielen zu erklären, wie der Paragraph in der Praxis interpretiert oder angewendet wird.
        3. Dazu anregen einen Kommentar oder eine Erläuterung zu erstellen, der die Bedeutung des Paragraphen mit praktischen Beispielen und der Relevanz für die Praxis deutlich macht.

        Beispielhafte Fragen:
        - "Was bedeutet der Paragraph in der Praxis und welche praktischen Beispiele existieren für seine Anwendung?"
        - "Erläutere die Bedeutung des Fachbegriffs aus dem Paragraphen und beschreibe anhand von Beispielen, wie er in der täglichen Praxis angewendet wird."
        - "Welche rechtlichen Implikationen hat dieser Paragraph und wie zeigt sich seine Relevanz durch Praxisbeispiele?"
        ...
        - Achte darauf, dass die Frage klar und präzise ist, um den Kommentar in eine juristische Erläuterung mit Beispielen zu lenken.
        """
        return {
            "messages": [
